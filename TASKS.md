...
- [x] **#17 End-to-end test student consumption flow** ✅
  New `backend/test_e2e_flow.py` (gitignored via `backend/test_*.py`) walks the entire pivot loop over real HTTP in ~30 seconds: register creator (timestamped email so reruns don't collide), generate via orchestrator, save to library (status→validated), register student, create class + batch, add student, assign content_item to batch, student fetches `/student/learn/{topic}` (lazy compute fires), student re-fetches (cached path is measurably faster), submit quiz score, verify progress endpoint reflects it. Plus two edge cases: not-assigned topic returns 404 with assignment-related copy; creator hitting `/student/learn` returns 403. Tiny test harness (no pytest dependency) with PASS/FAIL counters. ~5 Gemini calls per run (cheap to re-execute). No automated cleanup — test rows accumulate but use unique emails per run so they're inert; add a `--cleanup` flag later if the DB ever gets noisy. **First run after fixing the email-TLD and substring-matcher gotchas: 39/39 passed, 0 failed.** Loop is officially proven.
